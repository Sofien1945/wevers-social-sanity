...
const SearchItem = ({ videos }: { videos: Video[] }) => {
  const [isAccounts, setIsAccounts] = useState(false)
  const { allUsers }: { allUsers: IUser[] } = useAuthStore()
  const router = useRouter()
  const { searchItem }: any = router.query
      ...
      {isAccounts ? (
        <div className="md:mt-16">
          {searchedAccounts.length > 0 ? (
            ...
            <NoResults text={`No Account Results for ${searchItem}`} />
          )}
        </div>
      ) : (
        <div className="md:mt-16 flex flex-wrap gap-6 md:justify-start ">
          {videos.length ? (
            ...
            <NoResults text={`No Video Results for ${searchItem}`} />
          )}
        </div>
      )}
    </div>
...
export const getServerSideProps = async ({
  ...
  const res = await axios.get(`${BASE_URL}/api/search/${searchItem}`)
  return {
